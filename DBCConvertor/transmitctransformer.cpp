        ...
        if ((file.open(QIODevice::WriteOnly)))
        {
            ...
            for (int i=0; i<data_->msgboxes->size(); i++)
            {
                if (data_->msgboxes->at(i)->sender == data_->Transmitter){
                    ...
                    // если маска фжу формируется флаг - 1, иначе строка формируется заново
//                    bool is_mask = 0;
//                    QString mask = "0b11111111";

                    QString bitmask = "";
                    QMap<QString, QStringList> data;  // словарь для хранения данных
...
//                        // 1 случай: размер и позиция кратны 8
                        if (sgnl_size % 8 == 0 and sgnl_size >= 8 and sgnl_startbit % 8==0)
                        {
                            int array_pos = sgnl_startbit / 8;
                            int array_elems_amount = sgnl_size / 8;

                            // Ключ
                            QString key = "TX_can_data[" + QString::number(array_pos) + "] = ";

                            // Значение
                            QString value;
//                            QString type = sgnl->type_values.name != "uint8_t" ? "(uint8_t)" : "";
//                            bitmask = sgnl->type_values.name != "uint8_t" ? " & 0bFF" : " ";
...
//                            if (is_mask){
//                                // продолжить формирование маски
//                                bitmask += " | (vector->" + sgnl->name + " << " + QString::number(sgnl_startbit - array_pos * 8) + ")";
//                                int start_pos = sgnl_startbit % 8;
//                                for (int i=0; i<sgnl_size; i++){
//                                    mask.replace(9 - start_pos-i, 1, "0");
//                                }
//                            }
//                            else{
//                                // начать формирование маски
//                                is_mask = 1;
//                                tmp_array_pos = array_pos;
//                                text = "        TX_can_data[" + QString::number(array_pos++) + "] = ";

//                                if (sgnl_startbit % 8==0){
//                                    bitmask += " | (vector->" + sgnl->name + ");\n";
...
//                                else
//                                {
//                                    bitmask += " | (vector->" + sgnl->name + " << " + QString::number(sgnl_startbit - (array_pos-1) * 8) + ")";
//                                    int start_pos = sgnl_startbit % 8;
//                                    for (int i=0; i<sgnl_size; i++){
//                                        mask.replace(9 - start_pos-i, 1, "0");
//                                    }
